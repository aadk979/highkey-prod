...
function patchToCompact(patch: CustomizationMetaPatch): CompactCustomizationPatch {
  return [
    patch.product_id,
    patch.x_mm,
    patch.y_mm,
...
export function encodeCustomizationMeta(meta: CustomizationMeta): string {
  return JSON.stringify({
    v: CUSTOMIZATION_META_SCHEMA_VERSION,
    c: [meta.canvas.width_mm, meta.canvas.height_mm],
    f: meta.sides.front.patches.map(patchToCompact),
    b: meta.sides.back.patches.map(patchToCompact),
  } satisfies CompactCustomizationMeta);
}
